...
class YdhInventoryApi extends YdhApi
{
    const URL_WAREHOUSE_LIST = '/inventory/warehouse.json';
    ...
    public function warehouseList()
    {
        ...
        $url = $this->parseUrlParams(self::URL_WAREHOUSE_LIST, $this->params);
        Log::write($url, 'warehouseList');

        $res = json_decode(curl($url), true);
        if ($res['code'] != '200') {
            Log::write($res, 'warehouseListError');
